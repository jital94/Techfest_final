...
/*----------------------------------   */
   //Blocks
   #define redhuehigh 11
   #define redhuelow 0
   #define cyanhuehigh 40
   #define cyanhuelow 30
   #define yellowhuehigh 177
   #define yellowhuelow 168
   #define magnetahuehigh 177
   #define magnetahuelow 168
   #define backhuehigh 100
   #define backhuelow 72
   #define fronthuehigh 14
   #define fronthuelow 7
   
   //saturation
    #define redsathigh 200
    #define redsatlow 137
    #define yellowsathigh 255
    #define yellowsatlow 0
    #define cyansathigh 235
    #define cyansatlow 132
    #define magnetasathigh 255
    #define magnetasatlow 0
    #define backsathigh 255
    #define backsatlow 100
    #define frontsathigh 256
    #define frontsatlow 202
            ...
            {      h=ptemp[i*temp_erode->widthStep + j*temp_erode->nChannels+0];
                   ...
                   v=ptemp[i*temp_erode->widthStep + j*temp_erode->nChannels+2];
                               ...
                               {case 'r': //Place your condtion here for red
                                   {if((h<redhuehigh)&&(h>redhuelow)&& (s<redsathigh)&&(s>redsatlow) )
                                     poutput[i*output->widthStep + j]=255;
                                     else poutput[i*output->widthStep + j]=0;}
                                     break;
                                case 'c': //Place your condtion here for blue
                                     {if((h<cyanhuehigh)&&(h>cyanhuelow) && (s>cyansatlow) && (s<cyansathigh))
                                     poutput[i*output->widthStep + j]=255;
                                     else poutput[i*output->widthStep +j]=0;}
                                     break;  
                                case 'y': //Place your condtion here for yellow
                                     {if((h<yellowhuehigh)&&(h>yellowhuelow)&&(s>yellowsatlow)&&(s<yellowsathigh))
                                     poutput[i*output->widthStep + j]=255;
                                     else poutput[i*output->widthStep +j]=0;}
                                     break;  
                               case 'm': //Place your condtion here for yellow
                                     {if((h<magnetahuehigh)&&(h>magnetahuelow)&&(s>magnetasatlow)&&(s<magnetasathigh))
                                     poutput[i*output->widthStep + j]=255;
                                     else poutput[i*output->widthStep +j]=0;}
                                     break;  
                               case 'p': //Place your condtion here for yellow
                                     {if((h<backhuehigh)&&(h>backhuelow)&&(s<backsathigh)&&s>backsatlow)
                                     poutput[i*output->widthStep + j]=255;
                                     else poutput[i*output->widthStep +j]=0;}
                                     break;  
                               case 'o': //Place your condtion here for yellow
                                     {if((h<fronthuehigh)&&(h>fronthuelow)&&(s>frontsatlow&&s<frontsathigh))
                                     poutput[i*output->widthStep + j]=255;
                                     else poutput[i*output->widthStep +j]=0;}
                                     break;  
                               }
...
void sort_distance()
{
        ...
        for(int i=0;i<num_boxes-1;i++)
        {
                ...
                {
                        if(abs(puzzle[pos].distance-puzzle[j].distance)>17)
                            if(puzzle[pos].distance>puzzle[j].distance)
                             pos=j;
                             else;
                        else if(puzzle[pos].centroid.x<puzzle[j].centroid.x)
                             pos=j;
...
void create_delay(double delay)
{
     time_t timer;
     time_t timer2;
     double seconds;

     time(&timer);/* get begin time; same as: timer = time(NULL)  */
     //cout<<"Start\n";
     do
     {
      time(&timer2);  
